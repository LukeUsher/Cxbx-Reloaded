...
)
{
    ...
    if (pRects != nullptr) {
		...
		float aaX, aaY;
		GetMultiSampleScaleRaw(aaX, aaY);
		float Xscale = aaX * g_RenderTargetUpscaleFactor;
		float Yscale = aaY * g_RenderTargetUpscaleFactor;
...
void CxbxUpdateHostViewport() {
	...
	float Xscale = aaScaleX * g_RenderTargetUpscaleFactor;
	float Yscale = aaScaleY * g_RenderTargetUpscaleFactor;
